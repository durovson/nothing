...
оплату покупателя на отдельный адрес сделки, собирает её на центральный кошелёк
гаранта и удерживает до подтверждения покупателя. HTTP-часть работает на FastAPI, Telegram — на Aiogram 3.x,
...
5. PostgreSQL RPC атомарно фиксирует платёж и создаёт идемпотентную collection.
6. Deal-subwallet переводит весь остаток на WalletV5R1 гаранта №0. Только после
   успешного TonAPI trace сделка становится `paid`.
7. Центральный кошелёк удерживает деньги во время оказания услуги и спора.
8. Только покупатель может запросить release. Центральный кошелёк одной
   batch-транзакцией отправляет продавцу `D`, а сервису `D × 1%`.
9. Подписанные BOC и hash сохраняются до broadcast; после перезапуска операции
   безопасно восстанавливаются. Сделка завершается только по полному trace.

`deals.amount` — чистая сумма `D` продавцу. Счёт покупателя:
`D + D × ESCROW_FEE_RATE + TON_PAYOUT_FEE_RESERVE`. При `D=100`, комиссии 1%
и gas-резерве 0.01 покупатель платит `101.01 TON`; продавец получает ровно
`100 TON`, сервисный кошелёк — ровно `1 TON` с комментарием `Service fee`.
Gas расходуется из отдельной надбавки, а не уменьшает выплату продавцу.
Минимум сделки задаётся явно через `MIN_DEAL_AMOUNT` и равен `0.2 TON`.
...
`app/database/schema.sql`, `app/database/routines.sql` и
`app/database/permissions.sql`.
...
В Render Dashboard поля `Key` и `Value` заполняются отдельно. В `Value` нельзя
вставлять `KEY=`, Markdown-ссылки (`[https://...](https://...)`), угловые
скобки placeholders или внешние кавычки. Например, значение `TON_MNEMONIC`
должно состоять ровно из 24 слов без `"` в начале и конце. Приложение проверяет
эти ошибки при старте и прекращает запуск до подключения к внешним сервисам.
...
| `RENDER_KEEPALIVE_INTERVAL_SECONDS` | `600` | Интервал self-ping; для Render-шаблона `840` секунд = 14 минут. |
...
| `TON_PAYOUT_FEE_RESERVE` | `0.01` | Фиксированная gas-надбавка к счёту покупателя для collection и release. |
| `TON_GUARANT_PAYOUT_GAS_RESERVE` | `0.003` | Минимальный запас сверх `D + 1%` на центральном кошельке перед batch-выплатой. |
...
| `MIN_DEAL_AMOUNT` | `0.2` | Минимальная чистая сумма продавцу в TON. |
...
4. Последним выполните `app/database/permissions.sql`.
5. В `Project Settings -> API Keys` получите server-side secret key. Если проект
   использует старые ключи, допустим legacy `service_role`.
6. Запишите URL и ключ в `SUPABASE_URL` и `SUPABASE_KEY`.
7. Запустите приложение и проверьте `/readyz` и логи старта.
...
- `collection_attempts` — перенос с deal-subwallet на центральный гарант;
...
Неуспешные сделки со статусами `cancelled`, `creation_failed`, `collection_failed`,
`payout_failed` и `payout_bounced` удаляются фоновой задачей не позднее заданного
срока, максимум 30 дней. Связанные payment/collection/payout строки удаляются каскадно. Успешные сделки и
пользователи сохраняются.

При обновлении старые строки `paid` без `collection_attempts` и
`payout_attempts` получают `collection_failed`: база не может доказать, где
находятся их средства. Сначала сверьте каждую такую сделку on-chain; только
после этого оператор может вручную вернуть безопасную сделку в `collecting`.
Массово менять эти статусы нельзя — это создаёт риск повторной выплаты.
...
Он является центральным кошельком хранения и выплат. При старте адрес заново выводится из мнемоники, сети и workchain;
...
6. Сумма покупателя равна `D + 1% + 0.01 TON`. После проверки deal-subwallet
   использует send mode `128` только для collection на центральный гарант.
7. До подтверждения покупателя средства удерживаются на гаранте. Release
   использует две явные суммы без send mode `128`: продавцу `D`, сервису `1%`.
8. Перед подписью BOC проверяется центральный баланс `D + 1% + gas reserve`;
   недостаток средств оставляет release в очереди без частичной отправки.
...
даёт боту право подписи. Collection консолидирует их остатки в центральный
кошелёк, а release-очередь сериализует его `seqno`, чтобы выплаты не конфликтовали.
...
Это правило применяется и к collection на ещё не развёрнутый гарант-кошелёк:
дочерняя транзакция `uninit → uninit` может иметь `aborted=true` и
`cskip_no_state`, хотя non-bounced сообщение полностью зачислено. Collection
подтверждается только при точном destination/memo, положительном `credit_phase`,
отсутствии bounce и успешной исходящей action временного кошелька.
...
В `render.yaml` включён `RenderKeepAlive`: процесс сразу после старта и затем раз в 14 минут
...
Пинг раз в 15 минут находится на границе тайм-аута и ненадёжен. Используйте
14 минут или меньше. Подробнее:
...
| Supabase возвращает RLS/permission error | Используется ли server secret/service-role key, применены ли `schema.sql`, `routines.sql`, `permissions.sql`. |
| Оплата не найдена | Точный адрес сделки, сумма, memo, сеть и глубина `TON_TRANSACTION_SCAN_LIMIT`. |
| Collection зависла | TonAPI trace, баланс deal-subwallet, сеть и адрес гаранта. |
| Выплата зависла в submitted | Полный TonAPI trace, баланс центрального гаранта и оба адреса назначения. |
...
| Кошелёк комиссии | `SERVICE_FEE_WALLET` | Адрес точной выплаты `D × 1%` с комментарием `Service fee`. |
